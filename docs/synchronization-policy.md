...
It records a server-side backup name when deployment will replace that active script.
...
It then refreshes server state and retries the space check once.
...
It then restores that backup into the target script and leaves the backup script in place.

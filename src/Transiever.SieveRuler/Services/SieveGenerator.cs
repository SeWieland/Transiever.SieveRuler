...
public sealed class SieveGenerator : ISieveGenerator
{
        ...
        string sourceFile = "rules.json")
    {
        ...
        void AppendLf(string line = "") => sb.Append(line).Append('\n');

        AppendLf("# Generated by Transiever.SieveRuler.");
        AppendLf("# Review before installing on a Sieve-compatible mail server.");
        AppendLf($"# Source: {SieveStringEscaper.EscapeComment(sourceFile)}");
        AppendLf();
        if (capabilities.Count > 0)
        {
            AppendLf($"require {RenderStringList(capabilities)};");
            AppendLf();
        }
    ...
    private static SieveRuleBlock? RenderRule(RuleDefinition rule)
    {
        ...
        AppendLf(SieveProviderMetadata.RenderFlag(rule));
        AppendLf($"if {test.Test}");
        AppendLf("{");
        foreach (SieveAction action in actions)
            AppendLf(action.Command);
